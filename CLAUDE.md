...
**A buzz is deletable only by `wall_forget_boost`, and the flag alone is not what stops anything else.** `wall_boosts` is immutable and its trigger refuses every update and every delete. The one exception is a delete that carries the transaction local flag `wall.boost_undo`, which only `wall_forget_boost` sets, and that is also a row cast in the last thirty seconds on a date that has not sealed, which the trigger checks itself. Both halves are load bearing. A session setting is something anybody with raw SQL can set, so the flag on its own would make the word immutable a matter of good manners; the window and the seal are on the row and cannot be talked past. If you ever find yourself widening either half to make something pass, you are editing written history, which is the thing `docs/the-wall.md` refuses on every page.

**A source's own words never go inside a style rule.** `wallMarks` writes a `<style>` block and everything in it is generated text, which is why it is safe. A headline, a quotation or an outlet name is the source's wording: `escapeHtml` does not apply inside a CSS `content:` string, and one carrying the characters that end a style element ends the stylesheet and spills the rest of the page. Anything that has to show a source's words to one reader alone is drawn into the section on a request answered `no-store`, the way `foundBlock` and the anniversary are.

**`NotificationService.reschedule` clears every pending reminder and rebuilds the whole plan, so anything it needs it has to fetch itself.** A caller that forgets to hand over one kind of reminder does not add fewer reminders, it deletes every reminder of that kind the reader had waiting, and a reminder that is not registered does not announce itself: it simply never arrives. This is why `reachable` filters public figures inside the method, and why the sealed hive notes are read from `HiveNoteStore` there rather than passed in by any of the four call sites. If you add a new kind of reminder, give it a store and read it in `reschedule`; do not add a parameter. And anything that calls `reschedule` outside those four sites has to pass the real people list, because an empty one deletes every birthday on the phone.

**A new action in the `wall-write` Edge Function has to consume its grant.** The function writes one App Attest grant per call, before it dispatches, and the database function it calls is expected to spend it through `wall_require_attestation`. An action that does not spend it leaves a live grant behind, and the next direct call to `wall_cast_boost` from anywhere spends that one instead of attesting. This is why `wall_forget_boost` consumes a grant on its authenticated path even though it can only ever reach the caller's own row.
...
- **Commit as `-c user.name="Jason Page" -c user.email="72579902+jasonepage@users.noreply.github.com"`**, with the Claude co-author trailer. The number and the e matter: `jasonpage@users.noreply.github.com`, which this line said until September 22, 2026, belongs to a stranger's GitHub account and credited 472 commits to them until the history was rewritten, and split by what changed rather than by when it was written. A document touched by three pieces of work belongs in three commits, one section each.
