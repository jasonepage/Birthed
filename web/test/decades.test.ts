...
import { HIVE_DECADES_JS, decadeLine, decadeOf, decadeStanding, hiveDecades, yearOf } from "../src/decades.js";
...
test("zero buzzes: no line; one: a leader; three across two decades: the most first, older first on a tie", () => {
  ...
  assert.equal(decadeLine(one, "September 23", BEE, false), "The 1940s lead September 23 with the only buzz so far.");
  const three = day(stories.map((s) => (s.id === B ? { ...s, support: 2 } : s.id === A ? { ...s, support: 1 } : s)));
  assert.equal(decadeLine(three, "September 23", BEE, false), "The 1940s lead September 23 with 2 of 3 buzzes.");
  assert.deepEqual(decadeStanding(three).teams, [{ decade: 1940, buzzes: 2 }, { decade: 1860, buzzes: 1 }]);
  const tie = day(stories.map((s) => (s.id === B || s.id === A ? { ...s, support: 1 } : s)));
  assert.equal(decadeLine(tie, "September 23", BEE, false), "The 1860s and 1940s are level on September 23, 1 buzz each.");
  assert.deepEqual(decadeStanding(tie).leaders, [1860, 1940]);
  assert.equal(decadeLine(tie, "September 23", PLAIN, true), "The 1860s and 1940s were level on September 23, 1 tap each.");
});
...
test("the three live boards of September 21 to 23, 2026, as they stood", () => {
  ...
  assert.equal(decadeLine(s23, "September 23", BEE, false), "The 1860s, 1940s and 2020s are level on September 23, 1 buzz each.");
});
...
test("the block: nothing before the date, nothing with no buzzes, the line and the top three, and the tiles carry their decade", () => {
  ...
  assert.ok(sealed.includes("The 1940s led September 23 with 3 of 7 buzzes."));
});
...
test("the page's standing is the server's", () => {
  ...
  const page = api.standing([{ decade: 1940, support: 2 }, { decade: 1860, support: 1 }, { decade: null, support: 9 }, { decade: 2020, support: 0 }]);
  assert.deepEqual(page, { teams: [{ decade: 1940, buzzes: 2 }, { decade: 1860, buzzes: 1 }], total: 3, leaders: [1940] });
  ...
  assert.deepEqual(bent.teams, [{ decade: 1970, buzzes: 1 }]);
});
